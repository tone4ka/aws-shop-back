...
jest.mock("./utils/products", () => ({ addProductItem: (product) => addProductItemMock(product) }));
const publishToSNSMock = jest.fn();
jest.mock("./utils/publishToSNS", () => ({ publishToSNS: (products)=>publishToSNSMock(products) }));
